...
async def main(arg):
    ...
    for i in range(0, node):
        ...
        t_update = "event-driven" 
        evidence= ["distance","packet_loss_rate","response_time"]
        share = "0"
        
        x = {}
        x2 = {}
        for info in ["trustor", "trustee", "roles","lease_time","t_model","t_update","evidence","share"]:
            x[info] = eval(info)
    ...
    """
    #Delegate trust to a delegate
    protocol = await Context.create_client_context()
    await Delegate_all_roles(protocol,delegate_IP, all_trustee)
