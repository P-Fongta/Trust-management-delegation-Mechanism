...
class Delegate_All_Roles(resource.Resource):
    def __init__(self):
        super().__init__()

    def Delegate_all_roles_info(self, data):
    ...
    async def Delegate_all_roles(self,data):
        ...
        if T_update == "time-driven":
            ...
            while True:
                ...
                if measurement_count >= 600:  # Reset after 10 hours
                    measurement_count = 0
                # Sleep for 1 m
    ...
    async def render_post(self, request):
        ...
        print("\nRecv: Delegate_All_Roles from [...%s]"%(str(request.unresolved_remote.split(':', 4)[4]))) 

        #Store delegation info
        for item in data:
            self.Delegate_all_roles_info(item) 
        print("\nResp: to [...%s]"%(str(request.unresolved_remote.split(':', 4)[4]))) 
        print("\nStarting performing trust management on behalf of the delegators...")
        
        tasks = []
        for item in data:
            asyncio.create_task(self.Delegate_all_roles(item)) 
