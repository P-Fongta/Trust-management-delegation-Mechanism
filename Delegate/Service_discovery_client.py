...
async def main():
    ...
    local_IP = subprocess.getoutput("ip -6 addr show scope global | grep -oP 'inet6 \K[0-9a-fA-F:]+(?=/)'")
    ...
    x = {
      ...
      "e_storage": ["availability","distance","packet_loss_rate"],
      "e_collector": ["availability","distance","packet_loss_rate"],
      "d_maker": ["TD","TBS"],
    }
    
    await register(TSS_IP, local_IP, x)
    """
    #arguments for deregistration
    x = {
      "roles":  ["EC"]
    }
    await deregister(TSS_IP, local_IP, x)
    """
