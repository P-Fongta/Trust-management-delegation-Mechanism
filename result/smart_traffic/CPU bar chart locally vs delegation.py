...
for p in range(1,11):
    ...
    CI.append(2.262*(statistics.stdev(cpu)/math.sqrt(len(cpu))))
...
for p in range(1,11):
    ...
    CI2.append(2.262*(statistics.stdev(cpu2)/math.sqrt(len(cpu2))))
...
for p in range(1,11):
    ...
    for line in open(f'C:\data generator\\smart_traffic\\fuzzy\\result\\Do_locally{p}.txt', "r"):
        try:
            ...
            #continue
    print(cpu3)
    conf = st.t.interval(0.95, df=len(cpu3)-1, loc=np.mean(cpu3), scale=st.sem(cpu3)) 
    conf = tuple(map(lambda x: isinstance(x, float) and round(x, 2) or x, conf))
    C3interval.append(conf)
    cpu3_mean.append(float("{:.3f}".format(statistics.mean(cpu3))))
    cpu3_sd.append(float("{:.3f}".format(statistics.stdev(cpu3))))

    CI3.append(2.262*(statistics.stdev(cpu3)/math.sqrt(len(cpu3))))
print("Mean3 = %s \nSD = %s \nCinterval = %s"%(cpu3_mean,cpu3_sd,";".join(str(x) for x in C3interval)))
...
ax.set_title('CPU Time used by an and node ')
ax.set_xlabel('Number of target end nodes and fog nodes ')
ax.set_ylabel('CPU Time (Seconds)')

plt.bar(r, cpu3_mean, yerr=CI3, color = 'tab:green',align='center', alpha=0.8,capsize=8,
...
plt.bar(r + 0.25, cpu2_mean, yerr=CI2, color = 'tab:blue',align='center', alpha=0.8,capsize=8,
        width = width, edgecolor = 'black',
