...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\data generator\\smart_traffic\\fognode_onlyendnode\\power_measure\\power{p}-{z}.txt', "r"):
            try:
...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\data generator\\smart_traffic\\fognode_onlyfognode\\power_measure\\power{p}-{z}.txt', "r"):
            try:
                lines = [i for i in line.split()]
                W.append(float(lines[2])) 
                T.append(str(lines[3]))
...
plt.bar(r+0.25 , power2_mean, yerr=power2_sd, color = 'tab:orange',align='center', alpha=0.8,capsize=8,
        width = width, edgecolor = 'black',
