...
for p in range(1,11):
    ...
    for z in range(1,4):
        ...
        powerEachfile=[]
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health\delegation\\delegate collect evidence\\power_measure\\power{p}-{z}.txt', "r"):
            try:
...
for p in range(1,11):
    ...
    for z in range(1,4):
        ...
        power2Eachfile=[]
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health\delegation\\endnode collect evidence itself\\power_measure\\power{p}-{z}.txt', "r"):
            try:
                lines = [i for i in line.split()]
                W.append(float(lines[2])) 
                T.append(str(lines[3]))
