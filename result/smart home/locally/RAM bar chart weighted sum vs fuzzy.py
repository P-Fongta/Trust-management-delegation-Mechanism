    ...
    for z in range(1,11):
        for line in open(f'C:\data generator\e-health result 10 samples\locally\\weighted sum\\result\\Do_locally{p}-{z}.txt', "r"):
            try:
                ...
                #continue
    print(ramAllfile)
    ram_mean.append(float("{:.3f}".format(statistics.mean(ramAllfile))))
    ram_sd.append(float("{:.3f}".format(statistics.stdev(ramAllfile))))
    #calculate CI
    ...
    for z in range(1,11):
        for line in open(f'C:\data generator\e-health result 10 samples\locally\\fuzzy\\result\\Do_locally{p}-{z}.txt', "r"):
            try:
                ...
                #continue
    print(ram2Allfile)
    ram2_mean.append(float("{:.3f}".format(statistics.mean(ram2Allfile))))
    ram2_sd.append(float("{:.3f}".format(statistics.stdev(ram2Allfile))))
    #calculate CI
