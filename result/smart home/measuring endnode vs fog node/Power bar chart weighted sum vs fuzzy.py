...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health result 10 samples\\measuring endnode vs fog node\\measuring end node\\power\\power{p}-{z}.txt', "r"):
            try:
...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health result 10 samples\\measuring endnode vs fog node\\measuring fog node\\power\\power{p}-{z}.txt', "r"):
            try:
                lines = [i for i in line.split()]
                W.append(float(lines[2])) 
                T.append(str(lines[3]))
