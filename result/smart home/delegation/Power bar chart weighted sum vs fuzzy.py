...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health result 10 samples\\delegation\\endnode does not collect evidence\\power\\power{p}-{z}.txt', "r"):
            try:
    ...
    print(powerAllfile)
    power_mean.append(float("{:.2f}".format(statistics.mean(powerAllfile))))
    power_sd.append(float("{:.2f}".format(statistics.stdev(powerAllfile))))
    #calculate CI
...
for p in range(1,11):
    ...
    for z in range(1,11):
        W,T = [],[]
        for line in open(f'C:\\data generator\\e-health result 10 samples\\delegation\\endnode collects evidence\\power\\power{p}-{z}.txt', "r"):
            try:
                lines = [i for i in line.split()]
                W.append(float(lines[2])) 
                T.append(str(lines[3]))
    ...
    print(power2Allfile)
    power2_mean.append(float("{:.2f}".format(statistics.mean(power2Allfile))))
    power2_sd.append(float("{:.2f}".format(statistics.stdev(power2Allfile))))
    #calculate CI
