    ...
    for z in range(1,11):
        for line in open(f'C:\data generator\\e-health result 10 samples\\delegation\\endnode does not collect evidence\\result\\Do_delegation{p}-{z}.txt', "r"):
            try:
                ...
                #continue
    print(cpuAllfile)
    cpu_mean.append(float("{:.2f}".format(statistics.mean(cpuAllfile))))
    cpu_sd.append(float("{:.2f}".format(statistics.stdev(cpuAllfile))))
    #calculate CI
    ...
    for z in range(1,11):
        for line in open(f'C:\data generator\\e-health result 10 samples\\delegation\\endnode collects evidence\\result\\Do_delegation{p}-{z}.txt', "r"):
            try:
                ...
                #continue
    print(cpu2Allfile)
    cpu2_mean.append(float("{:.2f}".format(statistics.mean(cpu2Allfile))))
    cpu2_sd.append(float("{:.2f}".format(statistics.stdev(cpu2Allfile))))
    #calculate CI
